...
This chapter has been revised to compute Bonacich power centrality in Python instead of R (I implemented it in the `dcss` networks module). 

I will be making some relatively minor changes to this chapter in fall 2024 that will present more of a generative perspective on centrality. The idea will be to emphasize uncertainty and measurement issue to setup some of the more inferential content coming later in the book.

Finally, I'm going to switch from networkx to graph-tool in this chapter
:::
...
In order for some information that `173` has to reach `48` along the shortest path, it would have to first go through nodes `295`, `954`, and so on. What does this path look like? Let's visualize it. In @fig-15_01, the shortest path between `173` and `48` will be highlighted.
...
Let's plot all of these paths (@fig-15_02). 
...
Let's quickly visualize the distribution of betweenness scores with a histogram (@fig-15_04).
...
The code below computes current flow betweenness and then constructs a scatterplot (@fig-15_05) to compare the scores against the shortest path version.
...
If we supply networkx with the original in and out degree scores as node sizes, even the most popular and active nodes will be extremely tiny. Instead we will multiply every score by 20 to get them into a range of values that are large enough to use as node sizes in the visualization (@fig-15_06).
...
And now we'll do the same for outdegree (@fig-15_07). 
...
Let's take a look at the distribution of eigenvector centrality scores to degree (@fig-15_13).
...
Let's compare the Bonacich power centrality scores for different $\beta$ values to normalized degree centrality. @fig-15_bpc_ndc shows scatter plots of power centrality versus normalized degree centrality for a range of $\beta$ values, with each subplot corresponding to a different $\beta$.
