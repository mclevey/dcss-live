...
From here, we can plot (1) trends in the number of susceptible, infected, and removed nodes; and (2) the compartment deltas. The results are shown in Figure @fig-16_03.
...
Let's also take a quick look at the deltas for each state (Figure @fig-16_04). 
...
The results returned by `multi_runs()` are very similar to that returned by `iteration_bunch()`, only more deeply nested. The custom function from the utilities package, parses the output and produces a trend plot (Figure @fig-16_06).
...
This network is quite dense, so visualizing it is a bit tough. In Figure @fig-16_07, I overlay a contour plot to give an idea of where the density lies, and what kind of gradient it follows. 
...
Let's also inspect the degree distribution (Figure @fig-16_08).
...
The results are shown in Figure @fig-16_09.
...
Now let's model a simple contagion with $\beta = 0.01$, dramatically dropping the probability of transmitting an infection. The results are shown in Figure @fig-16_10.
...
As a quick check, let's quickly visualize the distribution of randomly sampled individual thresholds using an ecdf, which should be centered on .35 (see Figure @fig-17_05).
