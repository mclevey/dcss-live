...
Now that the data is loaded, let's create a scatterplot so we can see how it is distributed (@fig-26_01). 
...
Results are shown in @fig-26_02.
...
Take a look at what we can do by tightening our priors a little. The results are shown in @fig-26_03.
...
One of the most important steps in any Bayesian regression model involves checking your model's 'traces' to ensure that nothing went awry behind the scenes. ArviZ has some really nice built-in tools for this, shown for our trace pool model in @fig-26_04.
...
We'll follow in McElreath's footsteps and use 0.89, but there's no good reason why we couldn't use something like 0.83 or 0.79. The default for most ArviZ plots is 94%; having made our point, we'll leave the HDI intervals at their defaults from here on out. Results are shown in @fig-26_05.
...
If you inspect it, you'll find that the resulting `ppc` object is an `arviz.InferenceData` object. We can use this object directly with the `ArviZ` functions. In the code cell below, we use the `az.plot_ppc` function to produce a plot of our posterior predictive (@fig-26_06):
